    ...
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        {stats.map((stat, i) => (
          <div
            key={stat.label}
            className="bg-card rounded-xl border border-border p-4 sm:p-5 hover:border-border/80 transition-all animate-fade-in"
            style={{ animationDelay: `${i * 80}ms`, animationFillMode: "backwards" }}
          >
            <div className="flex items-center justify-between mb-2 sm:mb-3">
              <span className="text-xs sm:text-sm text-muted-foreground">{stat.label}</span>
              <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-xl bg-secondary flex items-center justify-center ${stat.color}`}>
                <stat.icon size={18} />
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-5">
        <div className="lg:col-span-2 bg-card rounded-xl border border-border p-4 sm:p-5 animate-fade-in" style={{ animationDelay: "200ms", animationFillMode: "backwards" }}>
        ...
        <div className="bg-card rounded-xl border border-border p-4 sm:p-5 animate-fade-in" style={{ animationDelay: "300ms", animationFillMode: "backwards" }}>
      ...
      {/* Recent Orders */}
      <div className="bg-card rounded-xl border border-border p-4 sm:p-5 animate-fade-in" style={{ animationDelay: "400ms", animationFillMode: "backwards" }}>
          ...
          <>
            ...
            <div className="block lg:hidden space-y-3">
              {orders.slice(0, 8).map((order) => (
                <div key={order.id} className="bg-secondary rounded-xl p-3 flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-foreground">{order.customerName}</p>
                    <p className="text-xs text-muted-foreground">{order.items.length} items · {order.paymentMethod}</p>
                    <p className="text-xs text-muted-foreground">{new Date(order.timestamp).toLocaleTimeString("en-IN", { hour: "2-digit", minute: "2-digit" })}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-vyellow tabular-nums">₹{order.total}</p>
