...
const Dashboard = () => {
  ...
  const stats = [
    { label: "Today's Revenue", value: `₹${todayRevenue.toLocaleString("en-IN")}`, icon: IndianRupee, change: "+12%" },
    { label: "Orders Today", value: todayOrders.length.toString(), icon: ShoppingBag, change: "+8%" },
    { label: "Avg Order Value", value: `₹${avgOrder}`, icon: TrendingUp, change: "+5%" },
    { label: "Total Orders", value: orders.length.toString(), icon: BarChart3, change: "" },
  ];
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((stat, i) => (
          ...
          >
            ...
            {stat.change && (
              <span className="text-xs text-accent flex items-center gap-0.5 mt-1 font-medium">
              ...
              <tbody>
                {orders.slice(0, 8).map((order) => (
                  <tr key={order.id} className="border-b border-border/50 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-2.5 px-3 font-bold tabular-nums">₹{order.total}</td>
                    ...
                    <td className="py-2.5 px-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-xs font-medium
                          ${order.status === "completed" ? "bg-accent/15 text-accent" : "bg-primary/15 text-primary"}`}
                      >
                        {order.status}
                      </span>
                    </td>
