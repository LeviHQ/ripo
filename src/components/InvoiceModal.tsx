...
export const InvoiceModal = ({ order, onClose }: InvoiceModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-foreground/40 flex items-center justify-center p-4 no-print" onClick={onClose}>
      <div
        className="bg-card rounded-2xl shadow-2xl w-full max-w-sm max-h-[90vh] overflow-auto animate-scale-in"
      ...
      >
        <div className="p-6">
          ...
          <div className="text-center mb-5 pb-4 border-b border-dashed border-border">
            <h3 className="text-xl font-bold text-primary tracking-tight">RIPO</h3>
            ...
            <tbody>
              {order.items.map((item) => (
                <tr key={item.id} className="border-b border-border/30">
                  ...
                  <td className="py-1.5 text-xs text-center text-muted-foreground tabular-nums">
