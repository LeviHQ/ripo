      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {menuItems.length} items across {categories.length - 1} categories
          </p>
        </div>
          ...
          <tbody>
            {filtered.map((item) => (
              <tr key={item.id} className="border-t border-border/50 hover:bg-muted/20 transition-colors">
                ...
                <td className="py-3 px-4 font-bold tabular-nums">₹{item.price}</td>
                <td className="py-3 px-4">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${item.isVeg ? "bg-accent/15 text-accent" : "bg-destructive/15 text-destructive"}`}>
                ...
                <td className="py-3 px-4 text-right">
                  ...
                  <button className="p-1.5 rounded-lg hover:bg-destructive/10 transition-colors text-muted-foreground hover:text-destructive ml-1">
                    <Trash2 size={15} />
                  </button>
                </td>
              </tr>
