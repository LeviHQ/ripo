  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
        ...
        <div className="flex gap-2 overflow-x-auto">
          {["all", "completed", "pending", "cancelled"].map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-2 rounded-xl text-xs sm:text-sm font-medium capitalize transition-all active:scale-[0.97] whitespace-nowrap
      ...
      <div className="block lg:hidden space-y-3">
        {filtered.map((order) => (
          <div key={order.id} onClick={() => setSelectedOrderId(selectedOrderId === order.id ? null : order.id)}
            className={`bg-card rounded-xl border border-border p-4 transition-all cursor-pointer ${selectedOrderId === order.id ? "border-primary/30" : ""}`}>
            ...
            <p className="text-xs text-muted-foreground mt-1">{new Date(order.timestamp).toLocaleString("en-IN")}</p>
            ...
            {selectedOrderId === order.id && (
              <div className="mt-3 pt-3 border-t border-border space-y-2 animate-fade-in">
                ...
                <div className="border-t border-border pt-2 space-y-1 text-sm">
                  ...
                  <div className="flex justify-between"><span className="text-muted-foreground">GST</span><span className="tabular-nums">₹{order.tax}</span></div>
                  <div className="flex justify-between font-bold text-base pt-1 border-t border-border"><span>Total</span><span className="text-vyellow tabular-nums">₹{order.total}</span></div>
                </div>
              </div>
            )}
          </div>
