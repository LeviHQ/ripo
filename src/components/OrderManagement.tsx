      ...
      <div className="flex gap-3 items-center flex-wrap">
        <div className="relative flex-1 max-w-xs">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-card border border-border focus:ring-2 focus:ring-primary/30 outline-none text-sm"
          />
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-muted/50">
                ...
                <th className="text-right py-3 px-4 text-muted-foreground font-medium text-xs uppercase tracking-wider"></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map((order) => (
                <tr
                  key={order.id}
                  className={`border-t border-border/50 hover:bg-muted/20 transition-colors cursor-pointer ${selectedOrderId === order.id ? "bg-primary/5" : ""}`}
                  onClick={() => setSelectedOrderId(order.id)}
                >
                  <td className="py-3 px-4 font-mono text-xs">{order.id}</td>
                  <td className="py-3 px-4">{order.items.length}</td>
                  <td className="py-3 px-4 font-bold tabular-nums">₹{order.total}</td>
                  <td className="py-3 px-4 capitalize text-xs">{order.paymentMethod}</td>
                  <td className="py-3 px-4">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs font-medium
                        ${order.status === "completed" ? "bg-accent/15 text-accent" : order.status === "pending" ? "bg-primary/15 text-primary" : "bg-destructive/15 text-destructive"}`}
                    >
                      {order.status}
                    </span>
                  </td>
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={7} className="py-12 text-center text-muted-foreground text-sm">No orders found</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        {selected && (
          <div className="w-80 bg-card rounded-xl border border-border p-5 space-y-4 animate-slide-in-right flex-shrink-0 self-start">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setSelectedOrderId(null)} className="p-1 rounded-lg hover:bg-muted">
                <X size={14} />
              </button>
            </div>
            ...
            <div className="border-t border-border pt-3 space-y-1 text-sm">
              <div className="flex justify-between">
                ...
                <span className="tabular-nums">₹{selected.subtotal}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">GST</span>
                <span className="tabular-nums">₹{selected.tax}</span>
              </div>
