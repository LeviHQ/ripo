...
import { Search, Plus, Minus, Trash2, Receipt, CreditCard, Smartphone, Banknote, ShoppingCart, X } from "lucide-react";
...
const POSScreen = () => {
  ...
  const cartPanel = (
    <>
      <div className="px-4 sm:px-5 py-3 border-b border-border space-y-2">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="font-bold text-foreground">Current Order</h2>
            <p className="text-xs text-muted-foreground">
              {cart.length} item{cart.length !== 1 ? "s" : ""} · {totalQty} qty
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowMobileCart(false)} className="lg:hidden p-1.5 rounded-lg hover:bg-secondary text-muted-foreground">
              <X size={18} />
            </button>
          </div>
        </div>
        ...
        {cart.length === 0 ? (
          ...
          cart.map((item) => (
            <div key={item.id} className="flex items-center gap-3 bg-secondary rounded-xl p-3 animate-scale-in">
              <img src={item.image} alt={item.name} className="w-10 h-10 sm:w-11 sm:h-11 rounded-lg object-cover flex-shrink-0" />
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] lg:h-screen">
      ...
      <div className="flex-1 flex flex-col p-3 sm:p-5 overflow-hidden">
        ...
        <div className="flex gap-2 mb-3 sm:mb-4 overflow-x-auto pb-1 scrollbar-hide">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 rounded-full text-xs sm:text-sm font-medium whitespace-nowrap transition-all active:scale-[0.97]
        ...
        {/* Items Grid */}
        <div className="flex-1 overflow-y-auto grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-2 sm:gap-3 content-start pr-1">
          {filtered.map((item, i) => (
            <button
              key={item.id}
              onClick={() => addToCart(item)}
              className="bg-card rounded-xl border border-border p-2 sm:p-3 text-left hover:border-primary/30 hover:shadow-lg hover:shadow-primary/5 transition-all duration-200 active:scale-[0.97] group animate-fade-in"
              style={{ animationDelay: `${Math.min(i * 30, 300)}ms`, animationFillMode: "backwards" }}
            >
              <div className="aspect-square rounded-lg overflow-hidden mb-2 sm:mb-2.5 bg-secondary">
              ...
              <div className="flex items-start justify-between gap-1">
                <div className="min-w-0">
                  <p className="font-medium text-xs sm:text-sm text-foreground leading-tight truncate">{item.name}</p>
                  <p className="text-vyellow font-bold text-xs sm:text-sm mt-0.5 sm:mt-1">₹{item.price}</p>
                </div>
                <span className={`w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-sm border-2 mt-0.5 flex-shrink-0 flex items-center justify-center ${item.isVeg ? "border-vgreen" : "border-vred"}`}>
      ...
      <button
        ...
        className="lg:hidden fixed bottom-4 right-4 z-30 w-14 h-14 bg-primary text-primary-foreground rounded-full shadow-lg shadow-primary/30 flex items-center justify-center active:scale-95 transition-transform"
      >
        <ShoppingCart size={22} />
        {totalQty > 0 && (
          <span className="absolute -top-1 -right-1 w-6 h-6 bg-vred text-white text-xs font-bold rounded-full flex items-center justify-center">
            {totalQty}
          </span>
        )}
      </button>
      ...
      {showMobileCart && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div className="absolute inset-0 bg-background/60 backdrop-blur-sm" onClick={() => setShowMobileCart(false)} />
          <div className="absolute right-0 top-0 bottom-0 w-full max-w-sm bg-card border-l border-border flex flex-col animate-fade-in z-10">
            {cartPanel}
          </div>
        </div>
      )}
      ...
      {/* Desktop cart panel */}
      <div className="w-[380px] bg-card border-l border-border hidden lg:flex flex-col flex-shrink-0">
        {cartPanel}
      </div>
