        ...
        <div className="flex items-center gap-4 mb-4">
          ...
          <div className="flex-1 relative max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-card border border-border focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none transition-all text-sm"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 content-start pr-1">
          {filtered.map((item, i) => (
            <button
              ...
              className="bg-card rounded-xl border border-border p-3 text-left hover:shadow-lg hover:shadow-primary/5 hover:border-primary/20 transition-all duration-200 active:scale-[0.97] group animate-fade-in"
              style={{ animationDelay: `${Math.min(i * 30, 300)}ms`, animationFillMode: "backwards" }}
            >
              <div className="aspect-square rounded-lg overflow-hidden mb-2.5 bg-muted">
                <img
                  src={item.image}
                  alt={item.name}
              ...
              <div className="flex items-start justify-between gap-1">
                ...
                <span className={`w-3.5 h-3.5 rounded-sm border-2 mt-0.5 flex-shrink-0 flex items-center justify-center ${item.isVeg ? "border-accent" : "border-destructive"}`}>
                  <span className={`block w-1.5 h-1.5 rounded-full ${item.isVeg ? "bg-accent" : "bg-destructive"}`} />
                </span>
              </div>
            </button>
          ))}
      ...
      <div className="w-[380px] bg-card border-l border-border flex flex-col flex-shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              <div key={item.id} className="flex items-center gap-3 bg-secondary/60 rounded-xl p-3 animate-scale-in">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="w-7 h-7 rounded-full bg-background border border-border flex items-center justify-center hover:bg-destructive/10 hover:border-destructive/30 transition-colors active:scale-95"
                  >
                ...
                <p className="text-sm font-bold text-foreground w-14 text-right tabular-nums">₹{item.price * item.quantity}</p>
              </div>
            ))
          )}
        </div>
          ...
          <div className="border-t border-border p-4 space-y-3 bg-card">
            ...
            <div className="flex items-center gap-2">
              ...
              <input
                ...
                className="w-14 px-2 py-1 text-xs rounded-lg border border-border text-center bg-background tabular-nums"
                min={0}
                max={100}
              />
            </div>
            ...
            <div className="space-y-1.5 text-sm">
              ...
              <div className="flex justify-between font-bold text-lg pt-2 border-t border-border">
                <span>Total</span>
                <span className="text-primary tabular-nums">₹{total}</span>
              </div>
            </div>
